	...
	<div>
		...
		<div class="page-rcont page-rcont-second">
			<div class="hbox" style="display:block;">
				...
				<!--am-collapse-item-->
					<div v-show="isHasData" v-for="(diagnoseConfig,diagnoseConfig_index) in diagnoseConfigs"
						 class="am-collapse-item">
						<div class="hd">
							<h3 :class="{'active':diagnoseConfig_index==0,'clearActive':true}" @click="slide_fun($event)">
								<i class="am-angle am-mr-10"></i>{{diagnoseConfig.catalogueName}}
									...
									<li @mouseenter="displayMeasures($event)" @mouseleave="closeMeasures($event)">
										<div class="config-measures">
											...
											<a href="javascript:void(0)" data-toggle="dropdown" role="button"
											   aria-haspopup="true" aria-expanded="true">{{diagnoseConfig.measures.length}}项 护理措施 <i class="am-angle am-ml-10"></i></a>
											<div class="am-dropdown" style="display:none">
												<div class="inner clearfix">
													...
													<!-- <p>1.通知医生。</p>
														<p>2.绝对卧床休息。【活动】</p>
														<p>3.开通静脉通路。</p> -->
													<p v-for="(measure,measure_index) in diagnoseConfig.measures">
												...
												<tr>
													<td v-for="(subset,subset_index) in component.subsets">
														...
														<div class="am-span-rdo-radius">
															<a v-show="positive.catalogueName!=''" class="active"
															   v-for="(positive,positive_index) in subset.items">{{positive.catalogueName}}</a>
														</div>
										...
										<td>
											<table class="am-tb-config-inner">
												<tr>
													<td>
														<div class="am-span-rdo-radius">
															<a v-show="goal.catalogueName!=''" class="active"
															   v-for="(goal,goal_index) in diagnoseConfig.goals">{{goal.catalogueName}}</a>
														</div>
													</td>
												</tr>
											</table>
										...
										<td>
											<table class="am-tb-config-inner">
												<tr>
													<td>
														<div class="am-span-rdo-radius">
															<a v-show="appraisals.catalogueName!=''" class="active"
															   v-for="(appraisals,appraisals_index) in diagnoseConfig.appraisals">{{appraisals.catalogueName}}</a>
														</div>
													</td>
												</tr>
											</table>
		...
		<div id="pop-content" style="display:none">
			<nursingConfigEdit ref='edit' @saveDiagnoseConfig="queryDiagnose" @showNav="showNav">
			</nursingConfigEdit>
		</div>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			displayMeasures: function (ev) {
				$(ev.target).children(".config-measures").children(".am-dropdown").css("display", "block")
				// this.isShowMeasure=true;
			},
			closeMeasures: function (ev) {
				$(ev.target).children(".config-measures").children(".am-dropdown").css("display", "none")
			},
			...
			delDiagnoseConfig: function (ev) {
				$(ev.target).siblings(".tool-confirm").css("display", "block");
			},
			// 新增诊断配置
			newDiagnoseConfig: function () {
				...
				this.$refs.edit.loadData(obj, "create", this.diaCatalogueNames, this.diagnosesDic, this.positiveDic, this.measuresDic, this.goalsDic, this.appraisalsDic);
				this.openPopConfig();
			},
			// 修改诊断配置
			viewDiagnoseConfig: function (diagnoseConfig) {
				...
				common.ajax({
					...
					callback: function (data) {
						...
						that.$refs.edit.loadData(data, "view", that.diaCatalogueNames, that.diagnosesDic, that.positiveDic, that.measuresDic, that.goalsDic, that.appraisalsDic);
						that.openPopConfig();
					}
				});
			},
			...
			slide_fun: function (ev) {
				...
				$(".clearActive").not(ev.target).removeClass("active");
				if (!$(ev.target).hasClass("active")) {
					$(ev.target).addClass("active");
					$(ev.target).parents(".am-collapse-item").find(".bd").stop(true, true).slideDown();
				} else {
					$(ev.target).parents(".am-collapse-item").find(".bd").stop(true, true).slideUp();
